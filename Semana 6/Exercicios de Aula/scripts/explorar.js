...
function gerarLista() {
  urls.map((url) => {
    const img = document.createElement("img"); //a cada volta no loop, em cada url, cria um atg img correspondente
    img.setAttribute("src", url); //cria uma img, com src url equivalente. Usa o url da linha 35
    document.getElementById("lista").appendChild(img);
    //joga um elemento filho dentro da id lista
  });
}

window.onload = gerarLista; //Assim qe a tela renderizar, iniciar esta função
